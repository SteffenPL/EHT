...
/**
 * Convert old inactive sentinel values in an event list.
 * Where start === Infinity && end === Infinity → set start: 0, end: -1
 */
function migrateEventSentinels(events: EventDefinition[]): void {
...
export function migrateV1_4_0toV1_5_0(params: EHTParams): EHTParams {
  ...
  if (migrated.general.default_events) {
    migrateEventSentinels(migrated.general.default_events);
  }
  ...
  for (const cellType of Object.values(migrated.cell_types)) {
    ...
    if (ct.events_v2) {
      migrateEventSentinels(ct.events_v2);
    }
  }
