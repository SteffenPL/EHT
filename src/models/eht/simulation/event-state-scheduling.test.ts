...
describe('dependent event state scheduling', () => {
  ...
  it('fires basal INM exactly in the cells where apical INM fires', () => {
    const events = [
      makeEvent('inm_contract_apical', {
        start: 0,
        end: 0,
    ...
    const params = makeParams(events);
    params.cell_types.control.INM = 0.5;
    ...
    for (let i = 0; i < 256; i++) {
      ...
      processV2Events(state, params, 0.1, new SeededRandom(`inm-half-step-${i}`));

      const apical = cell.event_states?.inm_contract_apical.has_fired === true;
      const basal = cell.event_states?.inm_extend_basal.has_fired === true;

      if (apical) apicalFired++;
      if (basal) basalFired++;

      expect(basal, `seed ${i}`).toBe(apical);
    }

    expect(apicalFired).toBeGreaterThan(0);
    expect(apicalFired).toBeLessThan(256);
    expect(basalFired).toBe(apicalFired);
  });
});
